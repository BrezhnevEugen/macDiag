...
- Live dashboard работает на реальном ЭБУ хотя бы одного целевого блока.
  Целевые машины и блоки (есть в наличии): **W221/X164**, двигатели
  **M273 → ЭБУ ME97 (ME9.7)** и **M276 → MED177 (MED17.7)**; плюс EZS221/EZS164,
  коробка 722.9 (VGS*), шлюз CGW. Все эти ЭБУ покрыты CBF и базой; декод и
  единицы работают (ME97: 790 формул/265 единиц; MED177: 1154/449).
  «Дизель» в прежней формулировке был случайным — декод не зависит от топлива.
...
Состояние после двенадцатого инкремента этапа 2 (sub-byte поля + bit_len):

- Schema v17. Колонка `bit_len` перестала быть мусором: теперь хранит
  авторитетную **ширину поля в битах** из пула (`width_bits`). Для sub-byte
  полей (`width_bits % 8 != 0`) `byte_len` обнуляется, ширина живёт в `bit_len`.
- Распределение `bit_len`: 16/8/32 (uword/ubyte/ulong), **1 у 12760** single-bit,
  2/3/4/… у мелких enum'ов; всего **14505 sub-byte полей** получили реальную
  битовую ширину.
- backend `_layout_data`: байт-выровненные поля режутся по байтам; настоящие
  sub-byte поля (≤8 бит) извлекаются битами по `bit_pos`; широкое
  невыровненное поле теперь **N/A**, а не грубый `resp[n:]`. `_raw_value` не
  фабрикует значение, если layout есть, но прочитать точно нельзя.
- `_extract_bits` обобщён до 32 бит. Добавлены тесты sub-byte извлечения.
- `_parse_diag_presentation` дочитывает extended-bitflag и берёт
  `TypeLengthBytesMaybe_21` как fallback, когда `TypeLength_1A` отсутствует:
  719 полей с отрицательной шириной (статусные флаги gearbox/park/keyless)
  теперь корректно 1-битные. `bit_len<=0` стало 0 — нет полей без ширины.
- `PRES_BLK*` ячейки: пул даёт каждой COL-ячейке реальную ширину (8/16/32 бита)
  и scale, поэтому `block` переопределяется в `ubyte/uword/ulong` счётчик.
  `raw_type='block'` стало **0** — опаковых hex-блоков больше нет; остаётся
  только 2D-сшивка COL-колонок в матрицу на уровне UI.
- stride-guard 97.9%.

Состояние после подготовки софт-части к Этапу 3 (без машины):

- **Маршрутизация** уже корректна в коде: `Session` держит одну 500k
  диагностическую CAN, `_channel_for` игнорирует per-ECU baudrate — адресация
  по CAN-id, шлюз маршрутизирует. Проверено: для целевых W221/X164 ЭБУ
  `group_ecu → _module_client → ecu_db` даёт верные id/протокол
  (ME97/MED177 0x7E0/0x7E8 uds, VGS 0x7E1, EZS221 0x612 kwp, CGW 0x602).
  Тест `test_w221_x164_ecu_routing_addresses` пинит это.
- **Парити simulator↔hardware**: `read_values` теперь единый путь — собрать
  request, guard, получить ответ (адаптер vs `_sim_response`), декодировать
  `_raw_value`+`_apply_output_formula`. Симулятор инвертирует формулу поля,
  кладёт raw по нужным битам с нужным порядком байт и декодирует обратно →
  баги масштаба/enum/порядка/sub-bit видны без машины. Заблокированные/
  непрочитанные → N/A с причиной (как на железе).
- **Debug-лог по параметру**: каждое значение несёт `read_req/read_resp/
  read_raw/read_sid/read_status/read_reason`; фронтенд показывает их в тултипе.

Состояние Этапа 4 (DTC → диагностический контекст), первый инкремент:

- Drill-down по DTC уже был (`/api/diag/context`: причины, чек-лист, media,
  связанные группы), но `linked` отдавал **все группы ЭБУ плоско**.
- `diag.keywords(code)` строит термины ошибки: описание DTC (de/en/ru) +
  стандартная OBD-область по номеру P-кода (misfire/EGR/throttle/…).
- `measurements.rank_measurement_groups()` ранжирует группы ЭБУ по числу
  совпадений терминов в title+job/alias; деградирует мягко (relevance=0).
- API ранжирует `linked.measurement`; фронтенд показывает «Рекомендуемые группы
  по ошибке» (подсвечены) отдельно от «Остальных». Примеры: P0300→Klopfsensor,
  P0120→Адаптация дроссельной заслонки, P0327→Klopfsensor.
- Критерий «клик по DTC даёт полезные группы, а не весь набор ECU» — выполнен.
  Осталось: обогатить термины из CBF-описаний DTC конкретного ЭБУ (сейчас
  `MB_DTC` всего 6 кодов) и привязать StarFinder media точнее по коду.
