...
| GET  | `/api/audit/actions` | append-only журнал изменяющих операций (success/error/blocked) |
...
- В `MACDIAG_MODE=hw` backend сам блокирует DTC clear и coding-записи, пока не
  ...
  от того, какой UI или HTTP-клиент отправил запрос. React-интерфейс дополнительно
  требует явного подтверждения, что DTC clear стирает всю память ошибок выбранного
  блока, а не один открытый код.
- Backend записывает каждую текущую изменяющую операцию (`dtc_clear`,
  `coding_apply`, `coding_write`) в append-only `data/action_audit.jsonl`, включая
  результат `success`, `error` или `blocked`. Журнал доступен только для чтения
  через `GET /api/audit/actions` и отдельную React-вкладку «Журнал»; он не
  заменяет coding backup. Каждое событие получает UUID, который API возвращает
  вместе с результатом операции и UI показывает пользователю.
